...
function createNewGraphViewerCard(id){
  return {
    id: `card-graphviewer-${id}`,
    sourceLink: [], //
    targetLink: [], 
    marginLeft: null, 
    marginTop: null, 
...
export default {
  ...
  state: {
    ...
    cards: [],
    component: () => import('@/components/viewer/graphviewer')
  }, 
  mutations: {
    ADD_COMPONENT(state){
      const nextIndex = state.nextAvailableIndex;
      state.nextAvailableIndex += 1
      state.cards.push(createNewGraphViewerCard(nextIndex));
    },
    ...
    SET_INPUTDATA(state, {link, inputData}){
      for(let i in state.cards){
        if(state.cards[i].id == link.target){
          ...
          state.cards[i].graphData = inputData;
        }
      }
    }, 
