...
export default {
  ...
  state: {
    ...
    component: () => import('@/components/loader/url'), 
  }, 
  mutations: {
    ADD_COMPONENT(state){
      const nextIndex = state.nextAvailableIndex;
      state.nextAvailableIndex += 1
      state.cards.push(createUrlCard(nextIndex));
    },
  ...
  actions: {
    ...
    async addUrl({commit, dispatch, state}, rawData){
        console.log('addurl function', rawData)
        for(let i in state.cards){
            if(state.cards[i].id == rawData.cardId && state.cards[i].data !== rawData){
                ...
                commit('LOAD_DATA', {id: rawData.cardId, data: rawData})
